...
# Hàm crawl dữ liệu từ thư viện HUIT
def crawl_data():
    url = "https://www.amazon.com/books-used-books-textbooks/b?node=283155"
    try:
        headers = {
        ...
        book_items = soup.select('.doc-item')  # Điều chỉnh selector dựa trên cấu trúc HTML thực tế
        
        for i, item in enumerate(book_items[:10], start=1):  # Lấy 10 sách đầu tiên
            title = item.select_one('.doc-title a')  # Điều chỉnh selector
            author = item.select_one('.doc-author')  # Điều chỉnh selector
            year = item.select_one('.doc-year')     # Điều chỉnh selector
            
            book = {
                "id": f"WEB{i}",
                "title": title.text.strip() if title else "Unknown",
                "author": author.text.strip() if author else "Unknown",
                "year": int(year.text.strip()) if year and year.text.strip().isdigit() else 2023,
                "genre": "Unknown",  # Website có thể không cung cấp thông tin này
                "quantity": 1
            }
            books.append(book)
        
        existing_books = read_json(BOOKS_FILE)
        existing_books.extend(books)
        write_json(BOOKS_FILE, existing_books)
        
        messagebox.showinfo("Thành công", "Đã crawl dữ liệu sách thành công!")
        show_books()
...
# Các hàm kiểm tra quyền truy cập
...
crawl_button = tk.Button(frame_search, text="Crawl Dữ Liệu", command=crawl_data)
crawl_button.grid(row=0, column=100, pady=10, padx=220, sticky="e")
